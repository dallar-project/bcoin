...
Chain.prototype.getTarget = async function getTarget(time, prev) {
  ...
  // Back 2 weeks
  let blockstogoback = pow.retargetInterval - 1;
  if ( (prev.height + 1) != pow.retargetInterval) {
    blockstogoback = pow.retargetInterval;
  }

  const height = prev.height - blockstogoback;
  assert(height >= 0);
...
Chain.prototype.retarget = function retarget(prev, first) {
  ...
  let newTarget = consensus.fromCompact(prev.bits);
  let oldTarget = newTarget;

  newTarget.imuln(actualTimespan);
  newTarget.idivn(targetTimespan);

  if (newTarget.gt(pow.limit))
    return pow.bits;
